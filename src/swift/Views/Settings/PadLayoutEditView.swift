...
struct PadLayoutEditView: View {
    ...
    var body: some View {
        Group {
            if editLandscape {
                // Landscape: editor fills entire view, toolbar floats on top
                ...
                VStack(spacing: 0) {
                    ...
                    GeometryReader { geo in
                        VStack(spacing: 0) {
                            ...
                            editorContent(areaW: geo.size.width, areaH: geo.size.height / 2)
                                .frame(height: geo.size.height / 2)
                        }
                    }
...
private struct DraggableGroup: View {
    ...
    var body: some View {
        ZStack {
            // Large transparent hit area with visible border
            ...
            // Actual button content (non-interactive in edit mode)
            groupContent
